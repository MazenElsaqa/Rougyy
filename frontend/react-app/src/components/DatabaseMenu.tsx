  ...
  return (
    <div>
      <button
        ...
        disabled={uploading}
        className="mb-1 flex w-full items-center gap-2 rounded-xl bg-primary-muted px-3 py-2 text-left text-[13px] font-semibold text-primary transition-all hover:shadow-sm disabled:opacity-60"
      >
        <svg viewBox="0 0 20 20" fill="none" className="h-4 w-4" aria-hidden="true">
          ...
          {uploading ? "Uploading…" : "+ Bring your own data"}
      </button>
      <input
        ref={fileInputRef}
        type="file"
      ...
      />
      {uploadError && <p className="px-3 pb-1 text-[11px] text-destructive">{uploadError}</p>}
